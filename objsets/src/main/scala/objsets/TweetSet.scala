...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(tweet: Tweet): Tweet = {
    if (elem.retweets > tweet.retweets) right.mostRetweetedAcc(left.mostRetweetedAcc(elem))
    else right.mostRetweetedAcc(left.mostRetweetedAcc(tweet))
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter( t => google.exists(i => t.text.contains(i)) )
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter( t => apple.exists(i => t.text.contains(i)) )
